...
## Tech Stack
- **Frontend:** React.js (Vite), TypeScript, Tailwind CSS, TanStack Query, React Router v6, React Hook Form + Zod, shadcn/ui, lucide-react.
- **Backend:** Node.js, Express.js, TypeScript, Zod for validation.
- **Database:** PostgreSQL.
- **ORM:** Prisma.
- **Authentication:** JWT (JSON Web Token) + bcrypt (for staff/admin only).
...
- **Naming:** 
  ...
  - Use `kebab-case` for file/folder names (e.g., `user-profile.tsx`) in both frontend and backend for cross-OS compatibility.
...
1. **No Student Login:** Students MUST NEVER require a password, login page, or account creation. Identity is verified via Bag Number + Mobile Number (and Magic Links + OTP).
